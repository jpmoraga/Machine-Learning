...
To be able to estimate the out-of-sample error, I randomly split the full training data (ptrain) into a smaller training set (ptrain1) and a validation set (ptrain2):
...
I am now going to reduce the number of features by removing variables with nearly zero variance or almost always NA and variables that will not be consider in the model.
...
NAvar <- sapply(ptrain1, function(x) mean(is.na(x))) > 0.95
ptrain1 <- ptrain1[, mostlyNA==F]
ptrain2 <- ptrain2[, mostlyNA==F]

# remove variables that will not be consider (X, user_name, raw_timestamp_part_1, raw_timestamp_part_2, cvtd_timestamp).
ptrain1 <- ptrain1[, -(1:5)]
ptrain2 <- ptrain2[, -(1:5)]
```
